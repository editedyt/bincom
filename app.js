...
const showPollUnits = async () => {
    ...
    await fetch('http://localhost/test%20Interview/server/get.php')
...
const getLga = async () => {
    await fetch('http://localhost/test%20Interview/server/getLga.php')
    .then(response =>  response.json())
    .then(response => {  
       if(response){
...
const sendlgaid = async () => {
    ...
    await fetch("http://localhost/test%20Interview/server/getpollUnit.php", {
        header: {
    ...
    .then(data => {
        if(data.success){
            ...
            return fetch("http://localhost/test%20Interview/server/getpollunit2.php", {
                header: {
                    'Content-Type': 'application/x-www-form-urlencoded'},
                method: "POST",
...
const addPollReult = async (loginForms) => {
    let vale = new FormData(loginForms);
    vale.forEach((value, key) => (vale[key] = value));

    console.log(vale)
    
    await fetch("http://localhost/test%20Interview/server/newPollResult.php", {
        header: {
            'Content-Type': 'application/x-www-form-urlencoded'},
        method: "POST",
        body: JSON.stringify(vale),
    ...
    .then(data => {
        if(data.success){
            ...
            msgContainer.innerHTML = `<p class="msg">
            ${data.msg}
            </p>`;
            setTimeout(() => {
                const msg = msgContainer.childNodes[0];
...
ul.onclick = (e) => {
    ...
    else if(ct.classList.contains("ar")){
